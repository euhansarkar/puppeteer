...
console.log(`see dirname`, dirname);

// Constants
const BASE_URL = "https://www.dealnews.com/categories/";
const OUTPUT_DIR = path.join(dirname, "categories");
...
async function main() {
  ...
  const newSelectedCategory = await categories?.filter(
    (category) => category.name === "Special Occasion"
  );
  ...
  await visitCategorySubcategory(page, newSelectedCategory);
